...
// POST /api/people/email/survey-invite
// Envía invitaciones o recordatorios para encuestas del módulo People.
// Modos:
...
export async function POST(request: Request) {
  try {
    ...
    // Verificar rol efectivo del módulo "people" (módulo > global)
    const { data: roleData } = await supabaseAdmin
      .from("user_roles").select("role").eq("user_id", user.id).single();
    const globalRole = roleData?.role ?? "viewer";
    const { data: moduleRoleData } = await supabaseAdmin
      .from("user_module_roles").select("role").eq("user_id", user.id).eq("module", "people").single();
    ...
    const surveyUrl = surveyId
      ? `${appUrl}/people/encuesta/${surveyId}`
      : `${appUrl}/people`;
    ...
    if (testEmail) {
      ...
      if (surveyId) {
        const survey = await prisma.peopleSurvey.findUnique({ where: { id: surveyId } });
        if (survey) {
          title = survey.title;
    ...
    console.error("[POST /api/people/email/survey-invite]", error);
