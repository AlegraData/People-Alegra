...
async function getPeopleEffectiveRole(userId: string): Promise<string> {
  ...
  return moduleRoleData?.role ?? globalRole;
}
...
export async function GET() {
  try {
    ...
    const role = await getPeopleEffectiveRole(user.id);

    // ── Admin / Manager: todas las encuestas con conteos ──────────────────
    if (role === "admin" || role === "manager") {
      const surveys = await prisma.peopleSurvey.findMany({
      ...
      const { data: assignmentRows } = surveyIds.length
        ? await supabaseAdmin.from("people_survey_assignments").select("survey_id").in("survey_id", surveyIds)
        : { data: [] };
      ...
      if (employee && surveyIds.length) {
        const myResponses = await prisma.peopleSurveyResponse.findMany({
    ...
    // ── Viewer: solo encuestas asignadas (o sin asignación) ─────────────
    const employee = await prisma.employee.findUnique({ where: { email: user.email! } });
    ...
    console.error("[GET /api/people/surveys]", error);
...
export async function POST(request: Request) {
  try {
    ...
    const survey = await prisma.peopleSurvey.create({
      data: {
        title: title.trim(),
    ...
    await supabaseAdmin.from("people_survey_assignments").insert(
    ...
    }).catch((err) => console.error("[auto-invite people]", err));
    ...
    console.error("[POST /api/people/surveys]", error);
...
async function sendInvitationsToAll(
  ...
  const surveyUrl = `${appUrl}/people/encuesta/${surveyId}`;
  const template  = {
