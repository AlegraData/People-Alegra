...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const survey = await prisma.peopleSurvey.findUnique({ where: { id } });
    if (!survey)
    ...
    const existing = await prisma.peopleSurveyResponse.findFirst({
    ...
    console.error("[GET /api/people/surveys/[id]/take]", error);
