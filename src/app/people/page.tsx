...
export default function PeoplePage() {
  ...
  const fetchSurveys = useCallback(async () => {
    try {
      const res = await fetch("/api/people/surveys");
      if (res.ok) {
        const data = await res.json();
        setSurveys(Array.isArray(data) ? data : []);
      }
    } catch (e) {
      console.error("Error cargando encuestas People.", e);
    }
  }, []);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (user) {
        try {
          const roleRes = await fetch("/api/auth/role?module=people");
  ...
  const handleUpdateSurvey = async (formData: SurveyFormData) => {
    ...
    try {
      const res = await fetch(`/api/people/surveys/${activeSurvey.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      });
  ...
  const handleDeleteSurvey = async (id: string) => {
    try {
      const res = await fetch(`/api/people/surveys/${id}`, { method: "DELETE" });
  ...
  const handleToggleStatus = async (id: string, currentIsActive: boolean) => {
    ...
    try {
      const res = await fetch(`/api/people/surveys/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        alert(`Error al ${currentIsActive ? "finalizar" : "reabrir"} la encuesta.`);
      }
    } catch {
      alert("Error de red.");
    }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
        <div>
          <h2 className="text-3xl font-black text-[#1e293b]">Encuestas People</h2>
          <p className="text-[#64748b]">Gestiona y participa en las encuestas del equipo de People.</p>
        </div>
