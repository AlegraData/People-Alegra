...
export default function SurveyParticipants({ survey, onBack, onSurveyUpdated }: SurveyParticipantsProps) {
  ...
  const fetchParticipants = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/people/surveys/${survey.id}/participants`);
  ...
  const handleReset = async (p: SurveyParticipant) => {
    ...
    try {
      await fetch(`/api/people/surveys/${survey.id}/responses/${p.employee_id}`, { method: "DELETE" });
  ...
  const handleRemove = async (p: SurveyParticipant) => {
    ...
    try {
      await fetch(`/api/people/surveys/${survey.id}/assignments/${p.employee_id}`, { method: "DELETE" });
