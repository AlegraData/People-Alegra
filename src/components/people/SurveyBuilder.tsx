...
function questionLabel(q: Question): string {
  ...
  if (q.type === "people")  return "Selección de persona";
        ...
        <div className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm">
          ...
          <div className="space-y-4">
            ...
            {questions.map((q, i) => (
              <div key={q.id} className="flex gap-4 items-start p-4 border border-slate-100 rounded-xl hover:border-slate-200 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {/* ── Selección de persona (People) ── */}
                  {q.type === "people" && (
                    <div className="mt-3 flex items-center gap-2 px-3 py-2.5 bg-rose-50 border border-rose-100 rounded-xl">
                      <span className="text-xl">👤</span>
                      <p className="text-xs text-rose-700 font-medium">Los participantes buscarán y seleccionarán a un integrante del equipo.</p>
                    </div>
                  )}
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              ...
              <button onClick={() => addQuestion("people")} className="text-xs font-bold bg-rose-50 hover:bg-rose-100 text-rose-600 px-3 py-2 rounded-lg transition-colors border border-rose-200">+ People</button>
            </div>
