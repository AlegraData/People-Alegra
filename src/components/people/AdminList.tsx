...
import { ClipboardList, Copy, Download, Plus, Pencil, Trash2, Users2, Lock, Unlock, Link, Check } from "lucide-react";
...
export default function AdminList({
  ...
  const handleCopyLink = (id: string) => {
    const url = `${window.location.origin}/people/encuesta/${id}`;
    navigator.clipboard.writeText(url).then(() => setCopiedId(id));
  };
                ...
                <tr key={s.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="py-4">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button
                        ...
                        className={`p-2 rounded-lg border transition-all ${
                          copied
                            ...
                            : "border-transparent hover:border-rose-200 hover:bg-rose-50 text-[#64748b] hover:text-rose-600"
                        }`}
                      >
                      ...
                      >
                        {confirming ? "¿Eliminar?" : <Trash2 className="w-4 h-4" />}
                      </button>
                    </div>
                  </td>
                </tr>
